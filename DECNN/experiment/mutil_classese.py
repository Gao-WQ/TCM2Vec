...
from utils import Init,Classes_Train
...
for mm in ['TextCnn','TextRCnn','DPCNN','TextRnn']:
    file_path = './data/multi_classes.xlsx'
    s_p = './save/'+ 'multi_classes/' + mm + '/' +time.strftime('%m-%d_%H.%M', time.localtime())
    ...
    params['require_improvement'] = 1000
    ...
    # params['emb_name'] = 'word2vec_embed_' + str(dim) + params['fn']
    # params['emb_name'] = 'bert_embed_' + str(dim) + params['fn']
    params['emb_name'] = 'FM_embed_50_100'
    params['embedding_pretrained'] = './embed/' + params['emb_name'] + '.txt'
    ...
    params['save_path'] = s_p + '_' +params['model_name'] + '_' + params['double_encoder'] + '_' + params['data_augment'] + '_' +params['relu'] + '_' + params['fastica'] + '_' + str(params['embedding_dim']) + '_' + params['emb_name']
    ...
    params, x_train,y_train,x_test,y_test,tests,labels = Init.Init_process(path=file_path, params=params)
    ...
    y_dev_t = torch.Tensor(y_test).long()
    ...
    if mm == 'TextCnn':
        model = TextCnn.TCM_1(params)
        print(model)

    elif mm == 'TextRCnn':
        model = TextRCnn.TCM_1(params)
        print(model)

    elif mm == 'DPCNN':
        model = DPCNN.TCM_1(params)
        print(model)

    elif mm == 'TextRnn':
        model = TextRnn.TCM_1(params)
        print(model)
    ...
    history, model = Classes_Train.Train(model, params=params, train_loader=train_loader_1, dev_loader=dev_loader, test_loader=test_loader)
